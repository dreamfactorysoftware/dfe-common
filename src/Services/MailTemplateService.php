...
class MailTemplateService extends BaseService
{
    ...
    public function send( $template, $data = array() )
    {
        ...
        $_settings = config( 'services.smtp-mail', array() );
        ...
        try
        {
            switch ( $_service )
            {
                ...
                default:
                    ...
                    $_transport = \Swift_SmtpTransport::newInstance( $_settings['server-name'], $_settings['server-port'] )
    ...
    {
        $_data = array();
        $_settings = IfSet::get( $replacements, '__smtp-mail.settings__', array() );
